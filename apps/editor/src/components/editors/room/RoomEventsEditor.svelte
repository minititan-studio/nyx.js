<script lang="ts">
    /**
     * RoomEventsEditor.svelte — script-file-based event panel for rooms.
     *
     * Mirrors TemplateEditor: loads/saves a TypeScript Room class on disk,
     * event picker inserts method stubs, Monaco shows the full file.
     * Room scripts are created on demand (scriptPath is optional on NyxRoom).
     */
    ...
    import { projectFilePath, currentProjectDir, currentProject } from '../../../stores/projectStore.js';
    import { signals } from '../../../stores/editorStore.js';

    interface Props {
        asset: NyxRoom;
        onscriptpathcreated: (path: string) => void;
    }
    let { asset, onscriptpathcreated }: Props = $props();
    ...
    // Track scriptPath locally so it updates immediately after creation
    let localScriptPath  = $state(asset.scriptPath ?? '');
    ...
    // ── File I/O ──────────────────────────────────────────────────────────────
    function absPath(sp: string): string | null {
    ...
    async function loadScript() {
        const abs = absPath(localScriptPath);
        if (!abs) {
            scriptError = null;
            scriptContent = '';
            return;
        ...
        try {
            ...
            scriptError = `Could not read: ${localScriptPath}`;
            scriptContent = '';
        }
    }
    ...
    function saveScriptDebounced() {
        ...
        saveTimer = setTimeout(async () => {
            ...
            try {
                ...
                console.error('[Nyx] Room script save failed:', err);
                if (lastSavedContent === toSave) lastSavedContent = '';
            }
        }, 500);
    }

    async function ensureScriptFile(): Promise<boolean> {
        if (localScriptPath) return true;
        ...
        try {
            ...
            onscriptpathcreated(scriptPath);
            await loadScript();
        } finally {
            creating = false;
        }
    ...
    function openInVSCode() {
        const dir = get(currentProjectDir);
        if (dir) electronAPI().vscode.open(dir);
    }
    ...
    $effect(() => {
        ...
        const uri   = monaco.Uri.parse(`file:///src/rooms/${asset.uid}.ts`);
    ...
    // Keep Monaco in sync when content changes from outside
    $effect(() => {
    ...
    // Reset when a different room is opened in this tab slot
    $effect(() => {
        if (asset.uid === prevUid) return;
        prevUid         = asset.uid;
        localScriptPath = asset.scriptPath ?? '';
        lastSavedContent = '';
        monacoModel?.dispose();
    ...
    // Also reload when scriptPath is set externally (e.g. after first creation)
    $effect(() => {
        const sp = asset.scriptPath ?? '';
        if (sp && sp !== localScriptPath) {
            localScriptPath = sp;
            loadScript();
        }
    });
    ...
    onMount(() => {
        return electronAPI().script.onFileChanged(({ relativePath }) => {
            ...
            if (normalized === localScriptPath.replace(/\\/g, '/')) loadScript();
        });
    });
    ...
    function pickerEventsForCategory(catKey: string) {
        return Object.entries(CORE_EVENTS).filter(([, def]) =>
            def.category === catKey && def.applicable.includes('room')
        );
    }

    // ── Event insertion ───────────────────────────────────────────────────────
    async function handleAddEvent() {
        const ok = await ensureScriptFile();
        if (!ok) return;
        pickerPending = null;
...
<div class="room-events">
    <!-- ── Left: method list + picker ── -->
    <div class="events-col">
        <div class="events-header">
            <span class="events-title">Methods</span>
            <div class="picker-wrap">
                <button class="add-event-btn" title="Add event method" disabled={creating}
        ...
        {#if localScriptPath}
            <div class="script-path dim small">{localScriptPath}</div>
        {/if}
        ...
        <div class="events-list">
            ...
            {/each}
            {#if detectedMethods.length === 0 && !scriptError && localScriptPath}
                <p class="dim small pad">No methods detected. Click + to add.</p>
            {/if}
            {#if !localScriptPath && !creating}
                <p class="dim small pad">Click + to create a room script.</p>
            {/if}
            {#if creating}
                <p class="dim small pad">Creating script file…</p>
            {/if}
            {#if scriptError}
    ...
    <!-- ── Right: Monaco (full .ts file) ── -->
    <div class="code-panel">
        <div class="code-header dim small">
            <Icon icon="feather:file-text" class="feather"/>
            <span class="code-path">{localScriptPath || 'No script file — click + to create'}</span>
            {#if localScriptPath}
        ...
        {#if scriptError && !scriptContent}
            <div class="script-error dim small pad">{scriptError}</div>
        {:else}
...
<style>
    ...
    .events-title {
        font-size: 0.78rem; color: var(--text-dim, #888);
    ...
    .add-event-btn {
        ...
        &:hover:not(:disabled) { background: var(--act, #1e2233); color: var(--acttext, #7ec8e3); border-color: var(--acttext, #7ec8e3); }
    ...
    .picker-args { padding: 0.5rem 0.6rem; }
    .picker-args-title { display: block; margin-bottom: 0.5rem; }
    .picker-args-actions { display: flex; gap: 0.4rem; margin-top: 0.5rem; }
    .picker-insert-btn {
        flex: 1; padding: 0.25rem 0.4rem; font-size: 0.78rem; border-radius: 3px; cursor: pointer;
        border: 1px solid var(--accent1, #446adb); background: var(--accent1, #446adb);
        color: #fff; transition: all 0.12s;
        &.secondary { background: transparent; color: var(--text-dim, #888); border-color: var(--border-bright, #333); }
        &:hover { opacity: 0.85; }
    }
    ...
    .picker-item {
        ...
        padding: 0.25rem 0.75rem; background: transparent; border: none;
    ...
    .event-item {
        ...
        padding: 0.3rem 0.4rem 0.3rem 0.6rem; cursor: pointer;
        font-size: 0.8rem; color: var(--text-dim, #888); border-left: 2px solid transparent;
        transition: all 0.1s;
        &:hover { background: var(--act, #1e2233); color: var(--acttext, #7ec8e3); border-left-color: var(--accent1, #446adb); }
    ...
    input[type="text"] {
        ...
        border-radius: 3px; color: var(--text, #e0e0e0); padding: 0.2rem 0.3rem;
        font-size: 0.8rem; width: 100%; box-sizing: border-box;
        &:focus { outline: none; border-color: var(--accent1, #446adb); }
    }
    select {
        background: var(--background-deeper, #111122); border: 1px solid var(--border-bright, #333);
        border-radius: 3px; color: var(--text, #e0e0e0); padding: 0.2rem 0.3rem; font-size: 0.8rem; width: 100%;
        &:focus { outline: none; border-color: var(--accent1, #446adb); }
    }
    ...
    .vscode-btn {
        flex-shrink: 0; width: 1.4rem; height: 1.4rem; padding: 0;
        background: transparent; border: 1px solid transparent;
        &:hover { border-color: var(--border-bright, #333); }
    }

    .code-area { flex: 1 1 auto; min-height: 0; position: relative; overflow: hidden; }

    .script-error { flex: 1 1 auto; }

    .dim   { color: var(--text-dim, #888); }
    .small { font-size: 0.78rem; }
    .pad   { padding: 0.5rem 0.75rem; margin: 0; }

    button.inline.square {
        cursor: pointer; background: var(--background, #1a1a2e); border: 1px solid var(--border-bright, #333);
        color: var(--text, #e0e0e0); border-radius: 4px; padding: 0; font-size: 0.82rem;
