    ...
    */
    export class Body {
        ...
        */
        ...
        /**
         * A Nyx copy that owns this Body object.
         */
        copy: Copy
    }
...
declare namespace matter {
    ...
    // Velocity helpers
    /** Sets only the horizontal velocity, preserving vertical (gravity keeps accumulating). */
    function setVelX(copy: Copy, vx: number): void;
    /** Sets only the vertical velocity, preserving horizontal. */
    function setVelY(copy: Copy, vy: number): void;
    /** Adds to the copy's current velocity on both axes. */
    function addVelocity(copy: Copy, dvx: number, dvy: number): void;
    /** Returns the copy's current velocity as a plain {x, y} object. */
    function getVelocity(copy: Copy): { x: number; y: number };
    /** Clamps the copy's velocity so |vx| ≤ maxX and |vy| ≤ maxY. */
    function clampVelocity(copy: Copy, maxX: number, maxY: number): void;
    /** Applies an instant velocity kick proportional to 1/mass (silent no-op on static bodies). */
    function impulse(copy: Copy, ix: number, iy: number): void;

    // Runtime property setters
    /** Sets air resistance (frictionAir) at runtime. 0 = no drag, 1 = full stop. */
    function setDrag(copy: Copy, drag: number): void;
    /** Sets the body's mass at runtime. */
    function setMass(copy: Copy, mass: number): void;
    /** Sets surface friction at runtime. */
    function setFriction(copy: Copy, friction: number): void;
    /** Makes the body static (immovable) or dynamic. Note: setting static zeroes velocity. */
    function setStatic(copy: Copy, isStatic: boolean): void;

    // Sleep control (requires engine created with enableSleeping: true)
    function sleep(copy: Copy): void;
    function wake(copy: Copy): void;
    function isSleeping(copy: Copy): boolean;

    /** Scales per-body gravity relative to the engine's global gravity. 0 = weightless, 2 = double gravity. */
    function setGravityScale(copy: Copy, scale: number): void;

    /** Returns true if the copy is resting on a solid surface below it. */
    function isGrounded(copy: Copy, distance?: number): boolean;

    /** Returns the physics group name assigned to this copy, or '' if none. */
    function getGroup(copy: Copy): string;
    /** Returns true if the copy belongs to the named physics group. */
    function inGroup(copy: Copy, groupName: string): boolean;
}
