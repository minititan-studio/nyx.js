...
const place = (function ctPlace() {
    ...
    // Tiles are rendered with center anchor for correct rotation pivot, but their
    // _tileOriginX/Y stores the original top-left corner used for collision shapes.
    const getOx = obj => obj._tileOriginX !== undefined ? obj._tileOriginX : obj.x;
    const getOy = obj => obj._tileOriginY !== undefined ? obj._tileOriginY : obj.y;

    const getSSCDShapeFromRect = function (obj) {
        const {shape} = obj,
              position = new SSCD.Vector(getOx(obj), getOy(obj));
    ...
    const getSSCDShapeFromLine = function (obj) {
        ...
        return new SSCD.Line(
            new SSCD.Vector(
                getOx(obj) + shape.x1 * obj.scale.x,
                getOy(obj) + shape.y1 * obj.scale.y
            ),
            new SSCD.Vector(
    ...
    const place = {
        ...
        getHashes(copy) {
            var hashes = [];
            var cx = getOx(copy), cy = getOy(copy);
            var x = Math.round(cx / place.gridX),
                y = Math.round(cy / place.gridY),
                dx = Math.sign(cx - place.gridX * x),
                dy = Math.sign(cy - place.gridY * y);
            hashes.push(`${x}:${y}`);
            if (dx) {
                hashes.push(`${x + dx}:${y}`);
                if (dy) {
