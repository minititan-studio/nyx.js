...
interface IPointer {
     ...
     * Use pointer.isButtonPressed(buttonName, pointer.buttons) to check
...
declare namespace pointer {
     ...
     * Note that it only changes when pointer works in the locking mode.
     ...
     * It can be used to reset the state of pointer, but note that hover events
