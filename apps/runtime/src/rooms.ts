    ...
    constructor(template: ExportedRoom, isRoot: boolean) {
        ...
        if (template) {
            ...
            for (let i = 0, li = template.bgs.length; i < li; i++) {
                ...
                // so we don't use backgrounds.add
...
function ensureUiOnTop(): void {
    const layer = uiModule.layer;
    // Use stage.children.includes rather than layer.parent: rooms.clear() sets
    // stage.children.length = 0 directly, leaving layer.parent stale.
    if (!(pixiApp.stage.children as pixiMod.DisplayObject[]).includes(layer)) {
        pixiApp.stage.addChild(layer);
    } else {
        pixiApp.stage.setChildIndex(layer, pixiApp.stage.children.length - 1);
    }
}
...
const roomsLib = (() => {
    ...
    const roomsLib = {
         ...
         * It is usually prefilled by Nyx Studio.
         * @catnipIgnore
         */
        ...
        remove(room: Room): void {
            ...
            // UI layer is managed by NyxUILayer assets via forceSwitch, not by isUi rooms.
         ...
         * from existing ones.
         * This room is added to the stage after all the other rooms.
         ...
         * Creates a new room and adds it to the stage, separating its draw stack
         * from existing ones.
         * This room is added to the stage before all the other rooms.
        ...
        merge(roomName: string): RoomMergeResult | false {
            ...
            for (const t of template.objects) {
                const c = templatesLib.copyIntoRoom(t.template, target, t.x, t.y, {
                    tx: t.scale.x || 1,
                    ty: t.scale.y || 1,
                    tr: t.rotation || 0,
                    scaleX: t.scale?.x,
        ...
        forceSwitch(roomName?: string): void {
            ...
            // Mount screen-space UI layer on top of the room
            uiModule.mount(template.ui ?? [], template.uiLayerNames);
            ensureUiOnTop();
            updateViewport();
        ...
        /**
         * The name of the starting room, as it was set in Nyx Studio.
         * @type {string}
         */
        starting: '/*!@startroom@*/'
    };
