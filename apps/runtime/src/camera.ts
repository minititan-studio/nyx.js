...
/**
 * This class represents a camera used by the Nyx engine.
 * Usually you won't create new instances of it, but if you need, you can substitute
 * the main camera with a new one.
 *
 ...
 * This is the base, unscaled value. Use camera.scale.x to get a scaled version.
 * To change this value, see the `width` property.
 * @property {number} height The width of the unscaled shown region.
 * This is the base, unscaled value. Use camera.scale.y to get a scaled version.
 * To change this value, see the `height` property.
...
export class Camera extends PIXI.DisplayObject {
    ...
    update(time: number): void {
        ...
        // follow === -1 means "no follow" in the exported room format
        if (!this.follow && roomsLib.current?.follow && roomsLib.current.follow !== -1) {
    ...
    realign(room: Room): void {
        if (!room.isUi) {
            console.error('[camera] An attempt to realing a room that is not in UI space. The room in question is', room);
            throw new Error('[camera] An attempt to realing a room that is not in UI space.');
        }
