// ⚠️⚠️⚠️ THIS FILE IS USED IN BOTH THE RUNTIME AND NYX STUDIO
// DO NOT import any other runtime files in this one, as it will import
// the rest of the runtime and break Nyx Studio.
//
// `import type` is ok.
...
import type {sound as pixiSound, filters as pixiSoundFilters, Filter, IMediaInstance, PlayOptions, Sound, SoundLibrary} from '@pixi/sound';
import type {webaudio} from '@pixi/sound/lib';
import type {ExportedSound} from '@nyx/shared';
...
/**
 * All the sound data objects exported from Nyx Studio, mapped by their asset name.
 * @catnipIgnore
 */
...
// Lazily populated on first use so PIXI.sound is guaranteed to be defined
const fxNamesToClasses = {} as {
    [T in fxName]: typeof pixiSoundFilters[T]
};
let fxNamesPopulated = false;
const ensureFxNames = () => {
    if (fxNamesPopulated) return;
    fxNamesPopulated = true;
    ...
    for (const fxName of fxNames) {
        fxNamesToClasses[fxName as fxName] = PIXI.sound.filters[fxName as fxName] as any;
    }
};
...
/**
 * Applies a method onto a sound — regardless whether it is a sound exported from Nyx Studio
