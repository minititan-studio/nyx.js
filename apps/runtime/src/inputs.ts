...
export class NyxAction {
     ...
     * This *does not* check whether this input method is supported by Nyx.
     *
    ...
    addMethod(code: string, multiplier?: number): void {
        if (this.methodCodes.indexOf(code) === -1) {
            ...
            throw new Error(`[inputs] An attempt to add an already added input "${code}" to an action "${this.name}".`);
        }
    }
    /**
    ...
    setMultiplier(code: string, multiplier: number): void {
        ...
        if (ind !== -1) {
            ...
            console.warn(`[inputs] An attempt to change multiplier of a non-existent method "${code}" at event ${this.name}`);
            // eslint-disable-next-line no-console
            console.trace();
        }
    }
 ...
 */
export const actionsLib: Record<string, NyxAction> = {};
export const inputsLib = {
    registry: inputRegistry,
    /**
     * Adds a new action and puts it into `actions`.
     *
     * @param name The name of an action, as it will be used in `actions`.
     * @param methods A list of input methods. This list can be changed later.
     * @returns {NyxAction} The created action
     */
    addAction(name: string, methods: Array<{
        ...
        if (name in actionsLib) {
            throw new Error(`[inputs] An action "${name}" already exists, can't add a new one with the same name.`);
        }
        const action = new NyxAction(name);
