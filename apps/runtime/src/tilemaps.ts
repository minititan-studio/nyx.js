...
import type {ExportedTilemap, ExportedTile, TextureShape} from '@nyx/shared';
...
export class Tilemap extends PIXI.Container {
     ...
     * and tile placement. It is usually used by Nyx Studio.
     */
    constructor(template?: ExportedTilemap) {
        ...
        if (template) {
            ...
            for (let i = 0, l = template.tiles.length; i < l; i++) {
                ...
                const frameTex = textures[tile.frame];
                const sprite = new Tile(frameTex);
                ...
                sprite.x = tile.x + frameTex.width * tile.scale.x * 0.5;
                sprite.y = tile.y + frameTex.height * tile.scale.y * 0.5;
                // Physics catmods need the top-left origin (tile data coordinates).
                (sprite as any)._tileOriginX = tile.x;
                (sprite as any)._tileOriginY = tile.y;
    ...
    addTile(
        ...
        if (this.cached) {
            throw new Error('[tiles] Adding tiles to cached tilemaps is forbidden. Create a new tilemap, or add tiles before caching the tilemap.');
        }
    ...
    cache(chunkSize = 1024): void {
        if (this.cached) {
            throw new Error('[tiles] Attempt to cache an already cached tilemap.');
        }
...
const tilemapsLib = {
    /**
     * Creates a new tilemap at a specified depth, and adds it to the main room (rooms.current).
