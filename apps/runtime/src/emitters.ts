...
import type {default as particlesMod, Emitter} from '@pixi/particle-emitter';
...
import type {ExportedTandem, ExportedTandems} from '@nyx/shared';
...
class EmitterTandem extends PIXI.Container {
     ...
     * @param tandemData The template object of the tandem, as it was exported from Nyx Studio.
    ...
    constructor(tandemData: ExportedTandem, opts: ITandemSettings) {
        ...
        for (const emt of tandemData) {
            ...
            if (opts.tint) {
                ...
                const colorList = (settings.behaviors[1].config as any).color.list;
            ...
            const textures = emt.texture ? resLib.getTexture(emt.texture) : [PIXI.Texture.WHITE];
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const inst = new PIXI.particles.Emitter(this as any, settings as any) as EmitterPatched;
            ...
            inst.initialDeltaPos = {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                x: (emt.settings as any).pos.x,
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                y: (emt.settings as any).pos.y
            };
     ...
     * according to either a UI ticker or delta.
     * @returns {void}
     */
    update(): void {
        if (this.stopped) {
...
const emittersLib = {
     ...
     * @param {string} name The name of the tandem template, as it was named in Nyx Studio.
