...
/**
 * @property {boolean} isUi Whether the timer uses deltaUi or not.
 * @property {string|false} name The name of the timer
 */
export class NyxTimer {
     ...
     * @param [uiDelta=false] If `true`, it will use `deltaUi` for counting time.
     * if `false`, it will use `delta` for counting time.
     */
    constructor(timeMs: number, name?: string, uiDelta = false) {
        if (!roomsLib.current) {
            throw new Error('[NyxTimer] Timers can only be created when a main room exists. If you need to run async tasks before the game starts, use vanilla JS\' setTimeout method.');
        }
    ...
    /**
     * Removes the timer from the game loop. This timer will not trigger.
     * @returns {void}
     */
    remove(): void {
...
const timerLib = {
     ...
     * to access it from `timer.timers`.
     * @returns {NyxTimer} The timer
     */
    add(timeMs: number, name?: string): NyxTimer {
        return new NyxTimer(timeMs, name, false);
    },
    /**
     ...
     * @returns The timer
     */
    addUi(timeMs: number, name?: string): NyxTimer {
        return new NyxTimer(timeMs, name, true);
    },
    /**
