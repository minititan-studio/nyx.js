...
import type {ExportedRoom, ExportedTemplate, TextureShape} from '@nyx/shared';
...
export interface ICopy {
     ...
     * either by placing it in a room with Nyx Studio or by calling `templates.copy`.
     */
    xstart: number;
    /**
     * The starting location of a copy, meaning the point where it was created —
    ...
    kill: boolean;
    /** Whether this copy is active (step/draw events fire). Managed via the active getter/setter. */
    _active: boolean;
...
/**
 * An instance of a Nyx template. Nyx cannot predict the base class
...
export const CopyProto: Partial<BasicCopy> = {
    ...
    onBeforeCreateModifier(): void {
        // Filled by Nyx Studio and catmods
        /*!%onbeforecreate%*/
        /*!@onbeforecreate@*/
    }
};
...
const assignExtends = (target: BasicCopy, exts: Record<string, unknown>) => {
    ...
    // Strip undefined values — assigning undefined to PIXI setters (e.g. tint) throws
    const cleanExts: Record<string, unknown> = {};
    for (const key in exts) {
        if (exts[key] !== undefined) {
            cleanExts[key] = exts[key];
        }
    }
    Object.assign(target, cleanExts);
    target.tint = tint;
};
 ...
 * augments it with Nyx Copy functionality.
...
const Copy = function (
    ...
    if (template) {
        ...
        // Forward any extra methods injected by the script bridge (_bridgeClasses in scripts.js).
        // The bridge copies prototype methods from the user's TypeScript class onto the template
        // definition object. These include lifecycle overrides (onContactAny, onTimer1, etc.)
        // that are not in the standard templateMixin above.
        {
            const mixinKeys = new Set(Object.keys(templateMixin));
            const skipKeys  = new Set([
                'name', 'depth', 'texture', 'baseClass', 'visible', 'behaviors',
                'blendMode', 'animationFPS', 'playAnimationOnStart', 'loopAnimation',
                'extends', 'onCreate', 'onStep', 'onDraw', 'onDestroy',
            ]);
            const tDef = template as unknown as Record<string, unknown>;
            for (const key of Object.keys(tDef)) {
                if (!mixinKeys.has(key) && !skipKeys.has(key) && typeof tDef[key] === 'function') {
                    (this as unknown as Record<string, unknown>)[key] =
                        (tDef[key] as (...a: unknown[]) => unknown).bind(this);
                }
            }
        }
        if (exts) {
...
export const makeCopy = (
    ...
    if (!(template in templatesLib.templates)) {
        throw new Error(`[templates] An attempt to create a copy of a non-existent template \`${template}\` detected. A typo?`);
    }
    ...
    const copy = new (baseClassToPixiClass[t.baseClass] as any)(t, exts) as BasicCopy;
...
const templatesLib = {
    ...
    /**
     * A map of all the templates of templates exported from Nyx Studio.
     * @catnipIgnore
     */
    ...
    copy(template: string, x = 0, y = 0, params: Record<string, unknown> = {}): BasicCopy {
        ...
        return templatesLib.copyIntoRoom(template, roomsLib.current, x, y, params);
    },
    /**
    ...
    copyIntoRoom(template: string, room: Room, x = 0, y = 0, params: Record<string, unknown> = {}): BasicCopy {
    ...
    exists(template: string): boolean {
        if (!(template in templatesLib.templates)) {
            throw new Error(`[templates] templates.exists: There is no such template ${template}.`);
        }
        return templatesLib.list[template].length > 0;
    },

    /**
     * Checks whether a given object is a Nyx copy.
