...
const uLib = {
    ...
    /**
     * The duration of one fixed timestep, in seconds.
     * Always constant (default 1/50 = 0.02s at 50 Hz).
     * Set once at startup from project settings; updated if fixedFps changes.
     */
    fixedTime: 1 / 50,
    /**
     * Get the environment the game runs on.
     * @returns {string} Either 'nyx.ide', or 'nw', or 'electron', or 'browser', or 'neutralino'.
     */
    getEnvironment(): 'nyx.ide' | 'nw' | 'electron' | 'browser' | 'neutralino' {
        if (window.name === 'Nyx debugger') {
            return 'nyx.ide';
        }
        if ('NL_OS' in window) {
            return 'neutralino';
    ...
    prect(x: number, y: number, arg: (BasicCopy | Array<number>)): boolean {
        ...
        if (arg instanceof Array) {
            ...
            if (arg.shape.type !== 'rect') {
                throw new Error('[u.prect] The specified copy doesn\'t have a rectangular collision shape.');
            }
    ...
    pcircle(x: number, y: number, arg: (BasicCopy | Array<number>)): boolean {
        ...
        if (arg.shape.type !== 'circle') {
            throw new Error('[u.pcircle] The specified copy doesn\'t have a circular shape');
        }
     ...
     * @returns {NyxTimer} The timer, which you can call `.then()` to
     * @catnipPromise both
     */
    wait(time: number): NyxTimer {
